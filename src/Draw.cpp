...
BOOL Flip_SystemTask(HWND hWnd)
{
	...
	const unsigned int delay_ticks[3] = {16, 17, 17};
    ++count;
    int delay = delay_ticks[count % 3]; // 20 for 50fps
	while (GetTickCount() < (timePrev + delay)) 
	{
		Sleep(1);
		if (!SystemTask())
			return FALSE;
...
BOOL StartDirectDraw(HWND hWnd, int wndSize)
{
	...
	switch (wndSize)
	{
		case WS_FULLSCREEN:
			...
			lpDD->SetDisplayMode(SURFACE_WIDTH, SURFACE_HEIGHT, 16);
			break;
		case WS_WINDOWED:
			fullscreen = FALSE;
	...
	backbuffer_rect.right = SURFACE_WIDTH;
	backbuffer_rect.bottom = SURFACE_HEIGHT;

	scaled_window_width = SURFACE_WIDTH;
	scaled_window_height = SURFACE_HEIGHT;
...
BOOL MakeSurface_File(LPCTSTR name, int surf_no) //TODO: ASM accuracy
{
	...
	int dst_x = 0;
	int dst_y = 0;
	int dst_w = bitmap.bmWidth;
	int dst_h = bitmap.bmHeight;
...
void PutNumber(int x, int y, int no)
{
	...
	for (int i = 0; i < 6; i++)
	{
		...
		if (v3 || i == 5)
			PutBitmap3(&grcFull, x + (8 * i), y, &rcNumber[v4], SURFACE_ID_GAMEINTERFACE);
	}
}